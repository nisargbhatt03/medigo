...
import { receptionNavItems } from './ReceptionSidebar';

export default function ReceptionHeader({ activeRecScreen, setActiveRecScreen, switchToSuperAdmin }) {
  const currentNav = receptionNavItems.find(item => item.id === activeRecScreen) || receptionNavItems[0];
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 pl-2 border-l border-slate-200">
          ...
          <div className="hidden sm:block text-left">
            <p className="text-xs font-semibold text-slate-800 leading-tight">Reception</p>
            <p className="text-[10px] text-slate-400">Front Desk</p>
          </div>
        </div>
      </div>
    </header>
