...
import { pharmacyNavItems } from './PharmacySidebar';

export default function PharmacyHeader({ activePharmaScreen, setActivePharmaScreen, switchToSuperAdmin }) {
  const currentNav = pharmacyNavItems.find(item => item.id === activePharmaScreen) || pharmacyNavItems[0];
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between sticky top-0 z-10 shadow-xs">
      ...
      <div className="relative w-72 sm:w-80 md:w-96">
        ...
        <input
          type="text"
          placeholder="Search medicine, invoice..."
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="relative p-2 text-slate-500 hover:text-slate-800 rounded-lg">
          ...
          <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-teal-600 rounded-full border-2 border-white" />
        </button>
        ...
        <div className="flex items-center gap-2 pl-2 border-l border-slate-200">
          ...
          <div className="hidden sm:block text-left">
            <p className="text-xs font-semibold text-slate-800 leading-tight">Pharmacist</p>
            <p className="text-[10px] text-slate-400">Sunrise Pharmacy</p>
          </div>
        </div>
      </div>
    </header>
